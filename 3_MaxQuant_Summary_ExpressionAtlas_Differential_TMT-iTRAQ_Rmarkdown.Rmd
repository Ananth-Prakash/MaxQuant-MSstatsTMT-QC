...
dir <- "/Users/ananth/Documents/MSstatsTMT_scripts_for_Andrew/"
...
idf  <- read.table( paste(dir,"idf.txt", sep="") , quote = "\"", header = TRUE, sep = "\t", stringsAsFactors = FALSE, comment.char = "#", fill=TRUE)

sample_channels <-read.table( paste(dir,"sample_channel_annotation.txt", sep="") , quote = "\"", header = TRUE, sep = "\t", stringsAsFactors = FALSE, comment.char = "#")
...
save(sample_channels, file = paste(dir,"table_sample_channels.rda", sep=""))
```
...
Proteincount_per_sample <- function(df){
  ...
  Intensity_data_cols <- c(grep("Reporter.intensity.[0-9].", colnames(df) ) )
...
number_of_samples_pre <- ncol(tmp[ ,c(grep("Reporter.intensity.corrected.[0-9].", colnames(tmp))) ])
...
# Apply Filter 2
# remove protein groups that have no mapping to ENSG gene IDs
tmp <- tmp[tmp$ENSG != "" ,]
tmp <- tmp[tmp$ENSG != "NA" ,]
...
number_of_samples_post <- ncol(tmp[ ,c(grep("Reporter.intensity.corrected.[0-9].", colnames(tmp))) ])
...
if((EXP_TYPE == "TMT") || (EXP_TYPE == "iTRAQ")){
...
proteincount_table_post$Samples <- gsub("Reporter.intensity.", "", proteincount_table_post$Samples, perl=TRUE) 
...
foldchange_Early_AD_NonDemented <- differential_foldchange[,c("EarlyOnset_Alzheimer.s.vs..EarlyOnset_Non_demented.foldChange", "EarlyOnset_Alzheimer.s.vs..EarlyOnset_Non_demented.pValue", "Gene.Symbol")]
foldchange_Early_AD_NonDemented$Contrast <- rep("EarlyOnset_Alzheimer.s.vs..EarlyOnset_Non_demented", nrow(foldchange_Early_AD_NonDemented))
colnames(foldchange_Early_AD_NonDemented) <- c("FoldChange", "Pvalue", "Gene.Symbol", "Contrast")

foldchange_Late_AD_NonDemented  <- differential_foldchange[,c("LateOnset_Alzheimer.s.vs..LateOnset_Non_demented.foldChange", "LateOnset_Alzheimer.s.vs..LateOnset_Non_demented.pValue", "Gene.Symbol")]
foldchange_Late_AD_NonDemented$Contrast <- rep("LateOnset_Alzheimer.s.vs..LateOnset_Non_demented", nrow(foldchange_Late_AD_NonDemented ))
colnames(foldchange_Late_AD_NonDemented ) <- c("FoldChange", "Pvalue", "Gene.Symbol", "Contrast")


foldchange <- rbind(foldchange_Early_AD_NonDemented, foldchange_Late_AD_NonDemented)
#threshold pvalue <= 0.05 and log2 foldchange 0.5 
foldchange$threshold = as.factor(foldchange$Pvalue <= 0.05 & (foldchange$FoldChange > 0.5 | foldchange$FoldChange < -0.5))
...
Quantification method:\hspace{25pt}      iTRAQ  
...
Differential analysis:\hspace{35pt}      MSstatsTMT v1.4.6  
...
Design:\hspace{82pt}                      3 replicates (AD_A, AD_B, AD_C)  
...
```{r sample_channels, echo = FALSE}
load(paste(dir,"table_sample_channels.rda", sep=""))
kable(sample_channels, booktabs = T) %>%
...
ggplot(peptidecount_table, aes(x=factor(Samples, levels = Samples), 
  ...
  #theme(axis.text.x = element_text(angle = 90, hjust = 1))+
  theme(legend.position="none")+
  ggtitle("Number of peptides mapped per group")
...
```{r Correlation_plot, fig.height = 6, fig.width = 6, echo = FALSE}
...
```{r pca_plot_data, fig.height = 6, fig.width = 6, echo = FALSE}
# PCA 
...
```{r foldchange, fig.height = 6, fig.width = 7, echo = FALSE} 
# Differential expression fold change
load(paste(dir,"FoldChange_plot_ExAtlas.rda", sep=""))
ggplot(foldchange, aes(x=FoldChange,  y = -log10(Pvalue), colour=threshold))+
...
Figure 7.  Differential protein expression. Proteins which show statistically significant differential expression (P-value <= 0.05 and foldchange 0.5 log2 scale) are coloured red.
